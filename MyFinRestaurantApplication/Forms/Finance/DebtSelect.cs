...
{
    public partial class DebtSelect : Form
    {
        public Entry selectedDebt;
        ...
        private async void UpdateCard()
        {
            ...
            var debts = await new Entry().OnLoadAsync();
            CreateTableLayoutGroup(debts, mainContainer, debts.Count);
        }

        public void CreateTableLayoutGroup(List<Entry> debts, Control parent, int count)
        {
            ...
            for (int j = 0; j < debts.Count; j++)
            {
                ...
                myTable.button.Text = string.Format("{0}\nЗадолженность: {1} {2}", i.GetEntryName, i.GetEntryDebt, currency);
        ...
        {
            if (((Guna2Button)sender).Tag is Entry aaa)
            {
                selectedDebt = aaa;
