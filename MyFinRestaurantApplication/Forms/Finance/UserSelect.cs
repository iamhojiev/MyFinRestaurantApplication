...
namespace ManagerApplication
{
    public partial class UserSelect : Form
    {
        ...
        private async void UpdateCard()
        {
            ...
            var users = await new User().OnLoadSalaryAsync();
            CreateTableLayoutGroup(users, mainContainer, users.Count);
        }

        public void CreateTableLayoutGroup(List<User> users, Control parent, int count)
        {
            ...
            for (int j = 0; j < users.Count; j++)
            {
                ...
                myTable.button.Text = string.Format("{0}\nЗарплата: {1} {2}", i.user_name, i.user_salary, currency);
        ...
        {
            if (((Guna2Button)sender).Tag is User aaa)
            {
                selectedUser = aaa;
