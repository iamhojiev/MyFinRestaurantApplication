...
{
    public partial class UserEarnings : Form
    {

        private User myUser;
        private SalaryType selectedType;

        public UserEarnings(User user)
        {
            InitializeComponent();
            myUser = user;
        ...
        {
            var types = new SalaryType().OnLoadEarnings();
        ...
        private async void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                bool success = await UpdateUserFinance(summa, selectedType.type);
                if (success)
                {
                    await BalanceSystem.Instance.AddSalaryOperation(selectedType.type, summa, myUser.user_id, txtComment.Text.Trim());
